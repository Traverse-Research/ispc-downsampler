...
impl Default for downsample_ispc::WeightVariables {
    fn default() -> Self {
...
impl WeightCollection {
    pub (crate) fn new(mut weights: Vec<CachedWeight>) -> Rc<Self> {
        let (starts, counts) : (Vec<_>, Vec<_>) = weights.iter().map(|w| {
            (w.start, w.coefficients.len() as u32)
        })
        .unzip();

        let weights = weights.drain(..).map(|w| {
            w.coefficients
        }).collect::<Vec<_>>();

        let weights_ptrs = weights.iter().map(|v| {
            v.as_ptr()
        }).collect::<Vec<_>>();

        Rc::new(Self { ispc_representation: downsample_ispc::WeightCollection {
            starts: starts.as_ptr(),
            weight_counts: counts.as_ptr(),
            values: weights_ptrs.as_ptr(),
        }, _starts: starts, _weight_counts: counts, _weights: weights, _weights_ptrs: weights_ptrs })
    }
