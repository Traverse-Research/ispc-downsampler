...
pub fn downsample(src: &mut Image, target_width: u32, target_height: u32) -> Vec<u8> {
    ...
    assert!((src.width * src.height * 4) as usize == src.pixels.len(), "TEMPORARY: The source image is not RGBA8: The expected size was {}, but {} was found", src.width * src.height * 4, src.pixels.len());
    ...
    unsafe {ispc::downsample_ispc::downsample_ispc::resample(src.width, src.height, src.width, target_width, target_height, src.pixels.as_mut_ptr(), output.as_mut_ptr())}
