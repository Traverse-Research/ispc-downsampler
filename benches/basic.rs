...
pub fn ispc_downsampler(c: &mut Criterion) {
    if let LoadResult::ImageU8(img) = load(Path::new("test_assets/square_test.png")) {
        ...
        // The PNG is sRGB-encoded, which the downsampler linearizes before filtering
        let src_fmt = if has_alpha {
            AlbedoFormat::Srgba8
        } else {
            AlbedoFormat::Srgb8
        ...
        c.bench_function(
            "Downsample sRGB `square_test.png` using ispc_downsampler",
            |b| b.iter(|| downsample(&src_img, target_width, target_height)),
        );
    }
}
